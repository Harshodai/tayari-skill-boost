...
## 2026-09-11 — Full-app flow-testing sweep (subagent) + fixes: raw markdown asterisks, fake-looking 999999 credit balance

### What was done
Dispatched a subagent to click through every major user flow live (Dashboard, Job Search, Cover Letter, Applications, Interview Prep, Review Queue, AutoPilot, Settings) against the real running stack. Confirmed the large majority of the app is genuinely solid: real LLM-generated cover letters (not mock fallback), real aggregated job listings from Greenhouse/Lever/Ashby/Workday/Remotive/arbeitnow, correct truthful-behavior compliance (refuses to fabricate compensation, correctly gates AutoPilot behind resume upload and interview prep behind an existing application), and no visible-error-state violations on the flows it could reach. Two real issues came back and were fixed:

1. **Literal unrendered markdown** (`**bold**` asterisks showing as plain text) on `/jobs/autopilot`'s hero copy. Grepping the same pattern found two more instances — `AgentPanel.tsx`'s "How it works" step list (also had a literal backtick-wrapped code string with the same problem) and `InterviewPrep.tsx`'s STAR-method explainer. None of these components render markdown; the `**word**` syntax was typed as if a markdown renderer would process it, but plain JSX strings never do. Fixed all three to real `<strong>`/`<code>` JSX.
2. **`999999` credit balance looks like a fabricated/placeholder number**: when `IsBillingEnabled()` is false (this deployment), `billing.go`'s `GetCreditBalance`/`DebitCredit` return a hardcoded `999999` balance and `999999` lifetime-purchased sentinel — a deliberate "unlimited" design for billing-disabled environments, but rendered to the user as a literal number with no indication it isn't real. This is exactly the class of thing this project's own truthful-product-behavior rule exists to catch, even though the underlying design (skip billing gating when disabled) is correct. Added an `Unlimited bool` field to `UserCreditBalance`, set `true` in both sentinel-returning branches, and threaded it through `src/api/credits.ts`'s `CreditBalance` type and every place that renders the raw number (`Dashboard.tsx`, `Credits.tsx`, `Settings/BillingSettings.tsx`) to show "Unlimited"/"Billing disabled for this deployment" instead of a bare `999999`.
