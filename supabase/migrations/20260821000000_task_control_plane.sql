...
CREATE POLICY task_runs_owner_access ON public.task_runs FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS task_plans_owner_access ON public.task_plans;
CREATE POLICY task_plans_owner_access ON public.task_plans FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS task_permissions_owner_access ON public.task_permissions;
CREATE POLICY task_permissions_owner_access ON public.task_permissions FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS action_proposals_owner_access ON public.action_proposals;
CREATE POLICY action_proposals_owner_access ON public.action_proposals FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS task_events_owner_access ON public.task_events;
CREATE POLICY task_events_owner_access ON public.task_events FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
