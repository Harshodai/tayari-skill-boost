...
- [x] **M0-03** Add a clean-worktree guard around frontend and macOS packaging commands. `scripts/require_clean_worktree.sh` now gates `build:release` and `desktop:build:mac:release`, with isolated clean/dirty checkout regression proof.
- [x] **M0-04** Record the current baseline: `docs/reports/repository-baseline-2026-08-25.md` records frontend lint warnings, frontend/Python/Go/security/promotion results, staging-plan evidence, and unavailable Docker/live-service prerequisites.
...
- [ ] **M8-01 / P0 — Instrument the paid funnel.** A privacy-safe named product-event contract now exists and is wired to the verified-user AutoPilot review-package entrypoint; the full visitor-to-paid funnel and payment acceptance measurement remain open.
...
- [ ] **M9-03 / P0 — Build the canonical application state machine.** Separate prepared, reviewed, candidate-confirmed, approved, attempted, receipt-confirmed, and externally verified states. Reconcile receipts before retry and prevent duplicate external actions. The pure canonical lifecycle and AutoPilot record/version integration are implemented and tested; durable database transition/reconciliation evidence remains open.
...
- [ ] **M9-05 / P1 — Build the canonical job identity and freshness ledger.** Deterministic normalized job identity, source URL/provider/title/company/location, and observed timestamp are now attached during discovery and used for prior-run deduplication. Durable freshness/expiry ledger and provider fixture evidence remain open.
- [ ] **M9-06 / P1 — Upgrade AI quality and cost observability.** ATS results now expose score decomposition, stuffing evidence/penalty, confidence, and explicit unsupported-claim uncertainty. Durable per-artifact model/prompt/cost trace coverage and the full evaluation corpus remain open.
