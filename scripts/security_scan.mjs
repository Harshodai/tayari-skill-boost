...
function scanEdgeFunctions() {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    // Functions generated by a framework that serves them itself (e.g.
    // @lovable.dev/mcp-js's `defineMcp`) never touch CORS/service-role
    // handling in the emitted app code — the library's own runtime does it,
    // verified against its bundled chunks (they set Access-Control-* headers
    // internally). Hand-editing these files is also a no-op: they're
    // regenerated and the edit is silently lost. Skip both heuristics for
    // them rather than false-positive on code nobody can or should change.
    const isFrameworkManaged = /^\/\/ AUTO-GENERATED by @lovable\.dev\/mcp-js/m.test(src);
    const usesServiceRole = !isFrameworkManaged && /SUPABASE_SERVICE_ROLE_KEY/.test(src);
    ...
    if (!isFrameworkManaged && !/corsHeaders/.test(src)) {
      add({
        scanner: "edge-functions",
        severity: "low",
