...
async function loadEvidence() {
  ...
  $('evidence-list').innerHTML = notes.slice(0, 5).map((note) => `<article class="evidence-item"><strong>${escapeHtml(note.title)}</strong><a href="${escapeHtml(note.url)}" target="_blank" rel="noreferrer">${escapeHtml(note.url)}</a><p>${escapeHtml(note.text).slice(0, 260)}</p><small>${escapeHtml(note.capturedAt)}</small></article>`).join('') || '<p class="muted">No local evidence captured yet.</p>';
}
...
$('sign-out').addEventListener('click', async () => { await send('sign_out'); activeTask = null; $('plan-card').classList.add('hidden'); await refresh(); });
...
$('plan-task').addEventListener('click', async () => { setAgentStatus('Building a reviewable plan...'); const result = await send('create_agent_task', { prompt: $('agent-prompt').value, mode: $('agent-mode').value, includeTabs: $('include-tabs').checked, tabId: current.tab?.id }); if (!result?.success) return setAgentStatus(result?.error || 'Could not create task.', 'error'); renderPlan(result); setAgentStatus('Plan ready for your approval.', 'ok'); });
$('approve-plan').addEventListener('click', async () => { const result = await send('approve_agent_plan', { taskId: activeTask?.id }); if (!result?.success) return setAgentStatus(result?.error || 'Plan approval failed.', 'error'); $('approve-plan').disabled = true; $('takeover-task').classList.remove('hidden'); setAgentStatus('Plan approved. Execution remains bounded by policy.', 'ok'); });
$('reject-plan').addEventListener('click', async () => { const result = await send('reject_agent_plan', { taskId: activeTask?.id }); if (!result?.success) return setAgentStatus(result?.error || 'Plan rejection failed.', 'error'); $('plan-card').classList.add('hidden'); activeTask = null; setAgentStatus('Plan rejected.', 'ok'); });
...
chrome.runtime.onMessage.addListener((request) => {
  if (request.action === 'selection_context' && request.text) {
    $('agent-prompt').value = `Explain this selected text and relate it to my job search:\n\n${request.text.slice(0, 4000)}`;
    setAgentStatus('Selection added to the prompt.', 'ok');
  }
});
...
async function refreshActiveTask() {
  ...
  const result = await send('get_agent_task_status', { taskId: activeTask.id });
  if (!result?.success || !result.task) return;
  activeTask = result.task;
  $('plan-scope').textContent = `Task status: ${result.task.status}. Plan approval and policy controls remain visible; final submission is blocked.`;
  if (['stopped', 'completed', 'failed', 'expired'].includes(result.task.status)) $('takeover-task').classList.add('hidden');
}
setInterval(() => void refreshActiveTask(), 6000);
chrome.runtime.onMessage.addListener((request) => { if (request.action === 'context_changed') void refresh(); });
