...
func TestApprovalNotificationProvidersFailClosedWhenUnconfigured(t *testing.T) {
	for _, key := range []string{"APPROVAL_EMAIL_ENDPOINT", "APPROVAL_EMAIL_API_KEY", "APPROVAL_EMAIL_FROM", "WHATSAPP_GRAPH_API_BASE_URL", "WHATSAPP_GRAPH_API_VERSION", "WHATSAPP_ACCESS_TOKEN", "WHATSAPP_PHONE_NUMBER_ID", "WHATSAPP_APPROVAL_TEMPLATE_NAME", "APPROVAL_SIGNING_KEY"} {
		t.Setenv(key, "")
	}
