...
func (s *Server) routesOmniSave(r chi.Router) {
	r.Group(func(r chi.Router) {
		...
		for _, prefix := range []string{"/api/v1", "/api"} {
			...
			r.Post(prefix+"/saves/capture/runs", s.handleOmniSaveProxyPost(omniSaveUpstreamPrefix+"/saves/capture/runs"))
			r.Get(prefix+"/saves/capture/runs", s.handleOmniSaveProxyGET(omniSaveUpstreamPrefix+"/saves/capture/runs"))
			r.Get(prefix+"/saves/capture/runs/{run_id}", s.handleOmniSaveProxyGETPath(omniSaveUpstreamPrefix+"/saves/capture/runs/", "run_id", ""))
			r.Post(prefix+"/saves/capture/runs/{run_id}/items", s.handleOmniSaveProxyBodyPath(omniSaveUpstreamPrefix+"/saves/capture/runs/", "run_id", "/items"))
			r.Post(prefix+"/saves/capture/runs/{run_id}/claim", s.handleOmniSaveProxyPostPath(omniSaveUpstreamPrefix+"/saves/capture/runs/", "run_id", "/claim"))
			r.Post(prefix+"/saves/capture/runs/{run_id}/heartbeat", s.handleOmniSaveProxyPostPath(omniSaveUpstreamPrefix+"/saves/capture/runs/", "run_id", "/heartbeat"))
			r.Post(prefix+"/saves/capture/runs/{run_id}/checkpoint", s.handleOmniSaveProxyBodyPath(omniSaveUpstreamPrefix+"/saves/capture/runs/", "run_id", "/checkpoint"))
			r.Post(prefix+"/saves/capture/runs/{run_id}/cancel", s.handleOmniSaveProxyPostPath(omniSaveUpstreamPrefix+"/saves/capture/runs/", "run_id", "/cancel"))
			r.Post(prefix+"/saves/capture/runs/{run_id}/finish", s.handleOmniSaveProxyBodyPath(omniSaveUpstreamPrefix+"/saves/capture/runs/", "run_id", "/finish"))
			r.Get(prefix+"/saves/capture/runs/{run_id}/items", s.handleOmniSaveProxyGETPath(omniSaveUpstreamPrefix+"/saves/capture/runs/", "run_id", "/items"))
...
func (s *Server) handleOmniSaveProxyGET(endpoint string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			slog.Error("[OmniSaveProxy] GET failed", "value", target, "error", err)
...
func (s *Server) handleOmniSaveProxyPost(endpoint string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			slog.Error("[OmniSaveProxy] POST failed", "value", endpoint, "error", err)
			writeOmniSaveProxyError(w)
			return
		}
...
func (s *Server) handleOmniSaveProxyBody(endpoint string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			slog.Error("[OmniSaveProxy] PUT failed", "value", endpoint, "error", err)
			writeOmniSaveProxyError(w)
			return
		}
		writeOmniSaveProxyJSON(w, http.StatusOK, result)
...
func (s *Server) handleOmniSaveProxyGETPath(prefix, parameter, suffix string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			slog.Error("[OmniSaveProxy] GET failed", "value", endpoint, "error", err)
			writeOmniSaveProxyError(w)
			return
		}
		writeOmniSaveProxyJSON(w, http.StatusOK, result)
