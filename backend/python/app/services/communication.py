...
class CommunicationGenerator:
    ...
    def generate(
        ...
        if comm_type == "follow-up":
            return CommunicationGenerator._follow_up(job_title, company_name, days_since)
        elif comm_type == "thank-you":
            return CommunicationGenerator._thank_you(job_title, company_name, recipient_name, discussion_points)
        elif comm_type == "negotiation":
            return CommunicationGenerator._negotiation(job_title, company_name, resume_text, offer_details)
        elif comm_type == "status-check":
            return CommunicationGenerator._status_check(job_title, company_name, days_since)
        else:
            return CommunicationGenerator._follow_up(job_title, company_name, days_since)

    @staticmethod
    def _follow_up(job_title: str, company_name: str, days_since: int) -> Dict[str, Any]:
        prompt = f"""Write a polite, concise follow-up email to {company_name} about the {job_title} application I sent {days_since} days ago.

Requirements:
- 3-4 sentences max.
- Express continued interest.
- Politely ask if there's any update on the timeline.
- Do NOT sound impatient or entitled.
- Sign off professionally.

Return ONLY the email body (no subject line)."""
        body = llm_complete(prompt, max_tokens=300, temperature=0.6)
        return {
    ...
    @staticmethod
    def _thank_you(job_title: str, company_name: str, recipient_name: Optional[str] = None, discussion_points: Optional[List[str]] = None) -> Dict[str, Any]:
...
Return ONLY the email body (no subject line)."""
        body = llm_complete(prompt, max_tokens=400, temperature=0.7)
        return {
    ...
    @staticmethod
    def _negotiation(job_title: str, company_name: str, resume_text: str, offer_details: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
Return ONLY the email body (no subject line)."""
        body = llm_complete(prompt, max_tokens=600, temperature=0.7)
        return {
    ...
    @staticmethod
    def _status_check(job_title: str, company_name: str, days_since: int) -> Dict[str, Any]:
...
Return ONLY the email body (no subject line)."""
        body = llm_complete(prompt, max_tokens=250, temperature=0.6)
        return {
