-- Durable provider-run state for asynchronous Apify research.
...
CREATE INDEX IF NOT EXISTS idx_external_research_runs_owner
    ON public.external_research_runs(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_external_research_runs_status
    ON public.external_research_runs(status, lease_expires_at);
CREATE INDEX IF NOT EXISTS idx_external_research_runs_provider
    ON public.external_research_runs(provider, provider_run_id);
...
CREATE POLICY external_research_runs_owner_select
    ON public.external_research_runs
