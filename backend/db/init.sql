...
CREATE POLICY user_credits_deny_client ON public.user_credits FOR ALL TO authenticated, anon USING (false) WITH CHECK (false);
DROP POLICY IF EXISTS credit_ledger_deny_client ON public.credit_ledger;
CREATE POLICY credit_ledger_deny_client ON public.credit_ledger FOR ALL TO authenticated, anon USING (false) WITH CHECK (false);
