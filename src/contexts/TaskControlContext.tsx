...
type TaskControlValue = { task: TaskRun | null; plan: TaskPlan | null; artifacts: TaskArtifact[]; events: TaskEvent[]; actions: ActionProposal[]; refresh: () => Promise<void>; approvePlan: () => Promise<void>; rejectPlan: () => Promise<void>; approveAction: (id: string) => Promise<void>; denyAction: (id: string) => Promise<void>; pause: () => Promise<void>; resume: () => Promise<void>; takeover: () => Promise<void>; stop: () => Promise<void>; };
...
export function TaskControlProvider({ taskId, children }: { taskId?: string; children: ReactNode }) {
  ...
  const refresh = async () => {
    ...
    const [nextTask, nextPlan, nextArtifacts, nextEvents, nextActions] = await Promise.all([
      ...
      listTaskArtifacts(taskId).catch(() => ({ artifacts: [] })),
      listTaskEvents(taskId),
      listActionProposals(taskId),
    ]);
    setTask(nextTask); setPlan(nextPlan); setArtifacts(nextArtifacts.artifacts); setEvents(nextEvents.events); setActions(nextActions.actions);
  };
