          ...
          <form onSubmit={handleRunAgent} className="space-y-3">
            <div className="flex gap-3">
              <Input
                ...
                placeholder="Enter a goal, for example: research this role and prepare a reviewed brief"
            ...
            <Input
              value={browserUrls}
              onChange={(e) => setBrowserUrls(e.target.value)}
              placeholder="Optional HTTPS URLs to browse (comma or newline separated; max 3)"
              className="bg-slate-900 border-slate-800 text-slate-100 placeholder:text-slate-500 focus:border-blue-500"
              disabled={loading}
              aria-label="Optional browser URLs"
            />
            <p className="text-[11px] text-slate-500">Pages are treated as untrusted evidence. External writes, submissions, credentials, and CAPTCHA actions remain blocked.</p>
          </form>
                ...
                <div key={st.step} className="p-4 rounded-lg bg-slate-950 border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                    <span className="text-emerald-400 font-bold">Step {st.step}: {st.action}</span>
                      <Badge variant="outline" className={agentData.status === 'completed' ? 'border-emerald-500/30 text-emerald-300 text-[10px]' : 'border-amber-500/30 text-amber-300 text-[10px]'}>{agentData.status === 'completed' ? 'Verified' : 'Needs review'}</Badge>
                  </div>
