...
export default function Omnisave() {
  ...
  const [pendingDeleteArticle, setPendingDeleteArticle] = useState<SavedArticleItem | null>(null);
  ...
  const filteredArticles = useMemo(() => {
    ...
    return articles.filter((article) => {
      ...
      const haystack = [article.title, article.author, article.category, article.nlp.summary, ...article.summary, ...article.tags, ...article.keyphrases, ...article.entities].join(" ").toLowerCase();
  ...
  const requestDeleteSource = (article: SavedArticleItem) => {
    if (backendUnavailable || deletingSourceId) return;
    setPendingDeleteArticle(article);
  };

  const handleDeleteSource = async () => {
    const article = pendingDeleteArticle;
    if (!article) return;
    ...
    <Layout>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5 text-primary" />{selectedArticle?.title || "Source workspace"}</DialogTitle><DialogDescription>{selectedArticle?.author ? selectedArticle.author + " · " : ""}Read, capture evidence, and connect this source to your career context.</DialogDescription></DialogHeader>
      ...
      <Dialog open={!!pendingDeleteArticle} onOpenChange={(open) => { if (!open) setPendingDeleteArticle(null); }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Delete “{pendingDeleteArticle?.title}”?</DialogTitle>
            <DialogDescription>This removes the source and any evidence cards or career context linked to it from your saved library. This cannot be undone.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setPendingDeleteArticle(null)}>Cancel</Button>
            <Button type="button" variant="destructive" onClick={() => void handleDeleteSource()}>Delete</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </Layout>
