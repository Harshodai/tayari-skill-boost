...
export interface TaskRun { id: string; title: string; objective: string; input_files?: TaskInputFile[]; status: TaskStatus; version: number; created_at: string; updated_at: string; stop_requested_at?: string; takeover_requested_at?: string; }
export interface TaskEvent { sequence_no: number; event_type: string; payload: Record<string, unknown>; created_at: string; }
export interface TaskPlan { task_id: string; version: number; steps: unknown[]; status: string; created_at: string; approved_at?: string; }
export interface TaskArtifact { id: string; task_id: string; artifact_type: string; title: string; content_type: string; body: string; provenance: Record<string, unknown>; created_at: string; updated_at: string; }
...
export const createTask = (input: { title: string; objective: string; input_files?: TaskInputFile[] }) => apiFetch<TaskRun>('/v1/tasks', { method: 'POST', body: JSON.stringify(input) });
...
export const getTaskPlan = (id: string) => apiFetch<TaskPlan>(`/v1/tasks/${encodeURIComponent(id)}/plan`);
export const listTaskArtifacts = (id: string) => apiFetch<{ artifacts: TaskArtifact[] }>(`/v1/tasks/${encodeURIComponent(id)}/artifacts`);
