...
| External providers | Adapter and gate code exists | Not verified live | Firecrawl, Apify, A2A, MCP, Gmail, Google Calendar, Google Drive, email, WhatsApp, billing, and other providers require real credentials and provider-specific staging evidence. |
...
| Durable automation | Tenant-bound definitions/runs/steps/events, Celery checkpoint worker, idempotency, expiry, risk tiers, and fail-closed tool boundary | Deterministic implemented; staged-only | Live worker restart/reclaim, schedule/webhook triggers, budget enforcement, two-tenant isolation, rollback, and tool-executor evidence. |
| Unified approvals | Exact action hashes, single-use digests, expiry, in-app decision routes, action-bound transitions, and approval audit events | Deterministic implemented; staged-only | Real replay/expiry/duplicate tests, provider-channel correlation, independent review, and downstream execution receipt evidence. |
| Approval email | Consent/preferences, provider-neutral adapter, idempotency, delivery states, signed webhook path, and in-app fallback | Disabled; not live-verified | Verified sender domain, provider staging delivery, bounce/complaint/suppression, revocation, redaction, outage, and rollback evidence. |
| Approval WhatsApp | Explicit opt-in, E.164 phone capture, template adapter, signed webhook path, delivery/read states, and in-app fallback | Disabled; not live-verified | Meta/provider approval, template, opt-in/opt-out, webhook signature, replay, delivery failure, quiet-hours, and staging evidence. |
