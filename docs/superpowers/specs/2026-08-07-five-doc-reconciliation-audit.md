...
| B8 | Real-time AI interview copilot (live transcription, STAR prompts) | PARTIAL-STUB | `live_interview_copilot.py:1,33,37`; `main.py:1027,1077,1189`; `routes_mvp.go:1949` (gate); `billing.go:156` (TierPro). **Frozen by founder decision** per `TAYARI_RUTHLESS_BRIEF.html:335` at audit time; unfrozen by user 2026-08-07 — streaming variant delivered 2026-08-10 (see V2 row) | L | Beyond §3.1 |
...
16. **B5–B13** — Networking, referrals, community, interview copilot (frozen at audit time; unfrozen by user 2026-08-07 — streaming delivered 2026-08-10, see V2 row), predictive ML, ethical AI. All PARTIAL-STUB or ASPIRATIONAL. Defer until Critical/High cleared.
...
## Addendum 2026-08-08 — 6th doc: Commercial Viability & Profitability (8 rows)
...
| # | Claim | Verdict | Evidence | Sev |
|---|---|---|---|---|
| V1 | Moat 1 — Referral Premium: LinkedIn mutual-connection mapping + automated referral workflow | PARTIAL-STUB | `recruiter_intelligence.py:16,21,46,70,92` (email-pattern heuristics + static `referral_intro_template`, no graph traversal); `referral_service.py` + `docs/superpowers/specs/2026-08-08-moat1-referral-draft-engine-design.md` + `docs/superpowers/plans/2026-08-08-moat1-referral-draft-engine.md` (LLM referral-draft engine, delivered 2026-08-08); `draft-outreach/index.ts:46` (drafts referral-ask, no automation, explicitly forbids LinkedIn scraping); `Networking.tsx:59,130,222`. **Remaining gaps: LinkedIn graph traversal and automated referral workflow still absent.** Overlaps B5/B6 | High |
| V2 | Moat 2 — Real-Time Interview Copilot (live audio transcription, WebSocket) | PARTIAL-STUB | `live_interview_copilot.py:1,33,37`; `main.py:1027,1077,1189` (3 endpoints, plain HTTP POST, no WebSocket/streaming); `routes_mvp.go:1949` (gate); `billing.go:156` (TierPro). **FROZEN** by founder per `TAYARI_RUTHLESS_BRIEF.html:581,637`. Overlaps B8. *Narrowed claim delivered 2026-08-10: transcript text over SSE (see Implementation update below) — live audio transcription and WebSocket support remain absent.* | High |
| V3 | Moat 3 — "Verified Human" badge (truthful resume + AI technical screening) | IMPLEMENTED (2026-08-07) | `candidate_verification` table (migration `20260807_verified_human_badge.sql` + supabase-local `21-` mirror, RLS deny-all); `verification_service.py` (truthfulness + screening moderators, `evidence: "resume_only"` — assessment, not proof); `routes_verification.go` (submit + status, `/api` + `/api/v1` parity, 503 `ai_service_unavailable`/`database_unavailable`); `Profile.tsx` badge card + Get-Verified dialog; `src/api/verification.ts` + tests; `verification` feature flag. **Remaining gap: no independent trusted-evidence signal yet — resume-only evaluations persist as unverified by design.** | Critical |
...
| V6 | Kill "Auto-Apply" branding → rename to "Outcome Pilot" or "Referral Engine" | DONE (2026-08-07) | Branding converged on **AutoPilot** (user decision — "let it be AutoPilot as present"): 35 "Apply Assist" instances → "AutoPilot" across 15 files; URLs (`/jobs/autopilot`) and "Auto-Apply" action verbs untouched; `src/config/branding.test.ts` static guard (recursive scan, zero "Apply Assist" in non-test src). Design: `docs/superpowers/specs/2026-08-07-v6-autopilot-branding-design.md`. **Remaining gap: none — single product name enforced by test.** | Medium |
...
- **V3 (Critical, new):** "Verified Human" badge — **IMPLEMENTED 2026-08-07** (see V3 row): persistence, scoring, submit/status routes, Profile controls, feature flag. The badge is live but honest-scope: resume-only evaluations persist as `unverified` until an independent trusted-evidence signal exists (by design — LLM scores from candidate-controlled text are assessment, not proof). **Do not market the badge as proof until that signal lands** (per the tayari-external-positioning skill: claims vs evidence).
- **V4 (Critical, new):** Pricing-tier mismatch — the $39/$99 "Agentic" tier structure in the doc doesn't exist in code (real: Free/Pro $19/Team-waitlist). Any pricing-page work must reconcile code↔doc↔marketing-copy (Landing.tsx $19/$49/$99 vs Pricing.tsx $19-only). **Not in scope** (user did not request).
- **V6 (Medium):** The "kill Auto-Apply branding" rename — **DONE 2026-08-07** (see V6 row): converged on AutoPilot, one name enforced by `branding.test.ts`. No remaining work.

These are **strategic/product** items, not B1-backend items. V3/V6 are delivered; V4 remains out of scope; the remaining moats (Moat-2 interview copilot — unfrozen, V7 Glass Box) belong in their own brainstorm cycles.

### Implementation update 2026-08-10 — V2 and V7 delivered

Both remaining moats from the 6th doc were implemented on 2026-08-10:

- **V2 — Moat 2 (Real-Time Interview Copilot):** unfrozen by user 2026-08-07; streaming variant delivered 2026-08-10. `backend/python/app/services/live_interview_copilot.py` `stream_live_copilot_hints` (SSE generator: `question_type` → `hints` → `star` → `metrics` → `[DONE]`; `LLMNotConfiguredError` → error event); `main.py:1610` (`POST /api/v1/interview/copilot/stream`); `backend/go/internal/api/routes_interview.go` (`handleInterviewCopilotHint`/`handleInterviewVoiceFeedback`/`handleInterviewCopilotStream` — SSE passthrough via `PostStream`, `requireFeature("interview_copilot")` gate kept); `src/api/ai.ts:234` `streamInterviewCopilotHints` (fetch + ReadableStream SSE parse); `InterviewBoard.tsx:69,120` (live mode — streamed hint panel). Plain-HTTP endpoints remain as fallback. **Remaining gap: still no live audio transcription — input is transcript text, not audio.** Overlaps B8.
- **V7 — Glass Box live browser feed:** `backend/python/app/services/browser_automation/agent.py:244` `stream_browser_agent` (screenshot SSE generator — base64 PNG + step/url/title per step, then `done`; `ai_service_unavailable` / `browser_agent_failed` error events); `main.py:1629` (`POST /api/v1/browser/automation/stream` → StreamingResponse); `backend/go/internal/api/routes_browser.go:23-24` (SSE passthrough via `PostStream`, `/api` + `/api/v1` parity); `src/api/browser.ts:17` `streamBrowserAgent` (fetch + ReadableStream SSE parse); `src/components/agent/AgentLiveView.tsx:201-247` — "Live browser feed" panel, progressive `<img>` screenshots + step counter + honest caption ("What the agent sees, per step — not a video stream"). **Remaining gap: none for the Manus-sense claim — screenshots stream live; it is a per-step snapshot feed, not video.** Overlaps C4 (row C4 still reflects the pre-feed snapshot).

Both moats are now implemented — V4 is the only remaining out-of-scope critical row from the 6th doc. Other rows remain open at their audit verdicts: V1 (PARTIAL-STUB — LinkedIn graph traversal + automated referral workflow absent), V2 (PARTIAL-STUB for the full claim — live audio transcription and WebSocket support still absent; the delivered transcript-text-over-SSE variant is covered above), V5 (PARTIAL-STUB — no "agentic" tier), V8 (ASPIRATIONAL — no cost-accounting in code).