...
|---|---|---|
| MIT-licensed schema package: `CandidateReference`, `CareerGoal`, `JobPosting`, `Approval`, `ApplicationReceipt`, and event-envelope types. | Candidate identity vault, profile graph, longitudinal memory, encryption/key management. | Candidate portability can be open; identity, encrypted data, and hosted reliability are the commercial service. |
| Consent/application state-machine specification and conformance fixtures. | Policy registry, policy authoring, risk detection, anti-abuse controls, and receipt reconciliation. | The protocol should be interoperable; managed enforcement and legal/policy operations create trust. |
