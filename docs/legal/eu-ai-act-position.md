...
- **Article 50 transparency obligations** (the AI Act's transparency chapter) are live for deployers from **2 August 2026**. Article 50 also imposes **provider** duties that may attach to JobTayari even as a candidate-side tool: Article 50(1) (AI systems that interact with individuals must disclose that the individual is interacting with an AI) and Article 50(2) (providers of AI systems generating synthetic audio, image, video, **or text** content must ensure outputs are marked machine-readable and detectable as artificially generated). JobTayari's resume/cover-letter generation and its chat/agent surfaces are within the Article 50(1)/(2) provider scope; compliance is NOT asserted until legal review confirms any exception and the applicable transparency control (e.g. an AI-output disclosure marker on generated application materials) is implemented and verified.
...
JobTayari is **not a deployer** under either regime: a "deployer" is a natural or legal person using an AI system "in the course of a personal non-professional activity" being excluded from the definition (Art. 3(4)), and a candidate applying for jobs with JobTayari falls on the personal-use side of that carve-out. It also remains a candidate-side tool with no employer-facing product, so the deployer-side obligations do not attach today. The deferral matters anyway — high-risk obligations are postponed, not dead; the gap to 2 December 2027 is the window in which a B2B pivot is cheapest from a compliance standpoint.
...
Candidate-side transparency is separate: every submission produces an immutable artifact of exactly what was sent (`submission_receipts` with the content, target, timestamp, and outcome). Receipts and the Terms of Service clause are **evidence and notice** — they let the candidate prove what was sent and put the candidate on notice of their own disclosure responsibility. They are **not** Article 50 compliance; deployer-side transparency obligations do not attach to JobTayari because the deployer of any AI system in this space is the party professionally using it — an employer screening candidates owes transparency to the candidate, and a candidate applying in a personal capacity is outside the Art. 3(4) deployer definition. Neither obligation runs to the tool provider.
