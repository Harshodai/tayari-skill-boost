...
- **Row B:** "Hit every endpoint listed in §1.2/§1.9 previously 404ing using valid authentication. Provide route-specific success evidence for each endpoint, asserting the expected handler response status and typed payload rather than treating any non-404 response with a body as fixed."
