...
| Verified saved jobs and pipeline tracking | Authenticated route probes confirm working candidate tracking surfaces | **A** | Candidate A/B isolation; create/update/delete; concurrency/duplicate submission; no status claimed as “submitted” without evidence; export/deletion compatibility |
| Unverified / dead job & pipeline flows (`routes_mvp.go`) | Dead flows caused by unregistered `routes_mvp.go` endpoints returning 404 | **B** until registered and validated | Ensure route registration under both `/api` and `/api/v1`, pass authenticated route probes, and verify DB persistence before promoting to activation A |
