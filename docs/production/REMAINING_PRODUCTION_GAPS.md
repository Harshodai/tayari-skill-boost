...
| PROD-016 | Learned-preference correction and retention controls | Users need to correct stale personalization without silent bias; production behavior depends on the new migration and owner-scoped persistence | Apply `20260825130000_add_memory_correction_controls.sql`; verify two-user correction, expiry, deletion, refresh, and purge in staging | **IMPLEMENTED LOCALLY; staging migration and two-user evidence pending** |
| PROD-017 | Retrieval and swarm quality evidence | Deterministic metric functions and explicit bounded recipes exist, but approved labeled fixtures and domain quality thresholds are not yet proven | Run versioned consent-safe benchmark; record NDCG/Recall@K/family precision; exercise child restart/reclaim/replay and review gates | **IMPLEMENTED LOCALLY; benchmark evidence pending** |
| PROD-018 | Durable swarm child lifecycle | Bounded specialist runs now persist owner-scoped child identity, attempt, status, timing, and digests without raw outputs; worker restart/reclaim evidence remains external | Apply mirrored child migration; kill/restart worker; verify reclaim, replay identity, cancellation, and no duplicate irreversible action | **IMPLEMENTED LOCALLY; staging worker evidence pending** |
| PROD-019 | Preparation outcome loop | Consent-gated progress metadata is now stored and reachable through the gateway and Outcomes UI; retention and outcome-quality evidence remain external | Apply mirrored outcome migration; verify consent denial, two-user isolation, expiry/purge, deletion, and approved outcome-quality reporting | **IMPLEMENTED LOCALLY; staging privacy/effectiveness evidence pending** |

## What is already locally closed

The current source also contains owner-scoped learned-preference correction controls, a versioned retrieval-metric contract, explicit review-first specialist recipes for the four supported task lanes, durable swarm-child lifecycle persistence, and consent-gated preparation-outcome capture. These are implementation closures only: they require migration application, approved benchmark fixtures, and staging worker/privacy evidence before they can be promoted to production claims.
