...
## Reconciled Status — 12 August 2026
...
| WS-04 | **Partially implemented; remains a release gate** | The current squad, truth gate, stored events, receipt model, and Omnisave durable-storage fail-closed behaviour are in place. | Focused Python regression coverage passed. | Replace process-local orchestration/cache fallbacks with a production durable run/event outbox, worker leases, cancellation acknowledgements, dead-letter handling, and recovery drills. |
| WS-05 | **Partially implemented; remains a release gate** | The application path applies a quality gate, approval fingerprint, ATS policy tier, LinkedIn prohibition, and receipt verification distinction. | Existing focused API and browser-path audits were reviewed in this hardening pass. | A true isolated per-candidate browser-worker lease, live takeover, credential handoff boundary, cancellation acknowledgement, and end-to-end portal test are required before claiming a Manus-equivalent browser computer. |
| WS-06 | **Designed; external integration gated** | The architecture now fixes safe channel roles: web-only final approval, Telegram/WhatsApp as opt-in notification and deep-link surfaces, and official Meta Cloud API—not WhatsApp Web automation—for production. | Research notes record the provider and Hermes adapter constraints. | Provider applications, webhook verification, opt-in/opt-out handling, encrypted credential store, delivery ledger, revocation and deletion drills must pass before activation. |
...
| WS-08 | **Implemented and locally verified; deployment process gate remains** | The repository now uses an explicit `vitest.config.ts` with a browser-like DOM, source aliases, deterministic cleanup, an offline-safe default fetch, and a runner-neutral test suite. Focused Python, Go, protocol, frontend API, type, production-build, whitespace, and baseline-security checks passed. | `npm test` — 32 files / 94 tests passed cleanly; `npm run build`, `npx tsc --noEmit`, and `npm run security:scan` passed. The scan returned 115 existing baselined findings and no new findings. | Require this complete suite, Go/Python checks, migration smoke tests, and deployment health checks in CI before merge or release. |
...
| P0 | Orchestrator | Introduce a durable event outbox, worker lease, cancellation acknowledgement, idempotency key, and dead-letter policy for every browser/connector action. | A forced worker crash and retry cannot duplicate an external action or lose an approval/revocation event. |
...
| P1 | Connectors | Build Telegram first, then official WhatsApp Cloud API as opt-in notification adapters with signed webhooks and a delivery ledger. | Opt-out, `/stop`, retry, ambiguous delivery, and deletion behaviours are automated and observable. |
