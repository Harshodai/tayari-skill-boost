...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conrelid = 'public.saved_sources'::regclass
          AND conname = 'uq_saved_sources_id_user'
    ) THEN
        ALTER TABLE public.saved_sources ADD CONSTRAINT uq_saved_sources_id_user UNIQUE (id, user_id);
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS public.omnisave_source_provenance (
    ...
    FOREIGN KEY (source_id, user_id) REFERENCES public.saved_sources(id, user_id) ON DELETE CASCADE
);
