...
-- DROP+CREATE (not just CREATE): init.sql and this file are top-level in
-- /docker-entrypoint-initdb.d, so Postgres's own entrypoint auto-runs them
-- once on its own PLUS once more via init.sh's explicit call — this must
