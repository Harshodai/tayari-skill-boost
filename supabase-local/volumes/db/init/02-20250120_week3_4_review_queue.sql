...
BEGIN
    IF OLD.status IS DISTINCT FROM NEW.status THEN
        INSERT INTO public.review_queue_history (
            ...
            NEW.application_id,
            NEW.user_id,
            -- 'saved' is not exclusively a review-queue approval outcome —
            -- handleUpdateApplicationStatus (routes_mvp.go) lets the kanban
            -- board set status to 'saved' from ANY prior status (e.g.
            -- 'applied' -> 'saved' when a user drags a card back), with no
            -- review-queue involvement. Only label it 'approved' when the
            -- transition actually came out of the review queue (OLD.status
            -- = 'review'); any other path landing on 'saved' falls through
            -- to the generic 'modified' action instead of a false 'approved'.
            CASE
                WHEN NEW.status = 'review' THEN 'queued'
                WHEN NEW.status = 'applied' THEN 'submitted'
                WHEN NEW.status = 'saved' AND OLD.status = 'review' THEN 'approved'
