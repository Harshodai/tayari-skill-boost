...
if [ "$1" = "--show" ]; then
    ...
    echo "New database password generated (hidden). Use --show to see it."
fi
echo ""
...
if ! echo "$db_srv_name" | grep -qE '^[a-zA-Z0-9._-]+$'; then
    echo "Error: Invalid service name format."
    exit 1
fi

if ! echo "$db_srv_port" | grep -qE '^[0-9]+$'; then
    echo "Error: Invalid port format."
    exit 1
fi

docker compose exec -T "$db_srv_name" psql -U "$db_admin_user" -d "_supabase" \
  -v ON_ERROR_STOP=1 \
  -v new_passwd="$new_passwd" \
  -v db_admin_user="$db_admin_user" \
  -v db_srv_name="$db_srv_name" \
  -v db_srv_port="$db_srv_port" \
  <<EOF
...
BEGIN
  IF EXISTS (
    ...
    SET config = jsonb_set(
      config,
      '{url}',
      format('"postgresql://%s:%s@%s:%s/postgres"', :'db_admin_user', :'new_passwd', :'db_srv_name', :'db_srv_port')::jsonb,
      false
    )
    WHERE type = 'postgres';
  END IF;
