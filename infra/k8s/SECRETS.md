...
| `WHATSAPP_GRAPH_API_BASE_URL`, `WHATSAPP_GRAPH_API_VERSION`, `WHATSAPP_ACCESS_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_APPROVAL_TEMPLATE_NAME`, `WHATSAPP_LINK_TEMPLATE_NAME`, `WHATSAPP_APP_SECRET`, `WHATSAPP_WEBHOOK_VERIFY_TOKEN`, `APPROVAL_SIGNING_KEY` | If WhatsApp approval delivery is enabled. | Use separate Meta-approved approval and link templates, explicit user opt-in and phone ownership confirmation, signed webhooks and button payloads, replay protection, opt-out, delivery/read reconciliation, and staging evidence. Keep both `CAPABILITY_WORKSPACE_NOTIFICATION_WHATSAPP` and `CAPABILITY_WORKSPACE_APPROVALS` false until the staging acceptance matrix passes. |
